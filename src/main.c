...
int main(int argc, char *argv[]) {
  ...
  ctx_conf_info conf = load_ctx_config_info("ctxs/ctx1.json");
  ...
  pauli_matrix *pms = load_ctx_configs("ctxs/batch", &out_count, NULL, 0);
  ...
  ctx_conf_info *configs_info = load_ctx_configs_info("ctxs/jbatch", &out_count);
  for (int i = 0; i < out_count; i++) {
    printf("-------------------\n");
